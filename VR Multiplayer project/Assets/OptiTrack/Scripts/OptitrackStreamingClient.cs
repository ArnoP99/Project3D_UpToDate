...
public class OptitrackStreamingClient : MonoBehaviour
{
    ...
    public string MulticastAddress = "127.0.0.1";
    ...
    void OnEnable()
    {
        ...
        if(MulticastAddress != "")
        {
            MulticastAddr = IPAddress.Parse(MulticastAddress);
        }
        else
        {
         MulticastAddr = null;
        ...
        try
        {
            ...
            switch (ConnectionType)
            {
                case ClientConnectionType.Unicast:
                    ...
                    m_client.Connect(connType, localAddr, serverAddr, 1510, 1511);
                    break;
                case ClientConnectionType.Multicast:
                default:
                    connType = NatNetConnectionType.NatNetConnectionType_Multicast;
                    m_client = new NatNetClient();
                    m_client.Connect(connType, localAddr, serverAddr, 1510, 1511, MulticastAddr);
                    break;
            }
